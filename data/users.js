...
export const createUser = async (userData) => {
  ...
  */
  ...
  if (cleanRole === "donor" || cleanRole === "distributor") {
    cleanOrg = h.checkAndThrowString(
      organizationName,
      "organizationName",
      2,
      200,
  ...
  */
  ...
  if (cleanRole === "donor" || cleanRole === "distributor") {
    const usersAtAddress = await users
      .find({ addressId: newAddressId })
      .toArray();

    for (const existingUser of usersAtAddress) {
      if (
        (cleanRole === "donor" && existingUser.role === "distributor") ||
        (cleanRole === "distributor" && existingUser.role === "donor")
      ) {
        throw new Error(
          `Registration failed: This address is already registered to a ${existingUser.role} account.`,
        );
      }
    }
  }
...
export const getAllUsers = async (filters = {}) => {
  ...
  if (filters.role) {
    ...
    if (
      ...
      !h.checkRoleValidity(filters.role)
    ) {
