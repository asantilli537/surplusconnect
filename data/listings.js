...
const validStatuses   = ['active', 'claimed', 'delivered', 'cancelled', 'expired'];
const validCategories = ['bakery', 'produce', 'dairy', 'meat', 'prepared', 'pantry', 'beverages', 'other'];
const validUnits      = ['pieces', 'trays', 'pounds', 'boxes', 'bags', 'gallons', 'servings', 'oz', 'lbs', 'cups', 'cans', 'containers'];
...
const parseAndValidateItems = (items) => {
  ...
  return items.map((item, idx) => {
    ...
    if (!validUnits.includes(unit.toLowerCase())) {
      throw new Error(`item[${idx}].unit must be one of: ${validUnits.join(', ')}`);
    }
...
const parseAndValidateCategory = (category) => {
  ...
  if (!validCategories.includes(clean.toLowerCase())) {
    throw new Error(`foodCategory must be one of: ${validCategories.join(', ')}`);
  }
...
const calculatePriorityScore = (expirationTime, items) => {
  const hoursUntilExpiry = (new Date(expirationTime) - new Date()) / (1000 * 60 * 60);
...
export const createListing = async (donorId, listingData) => {
  ...
  const cleanDesc  = description
    ? h.sanitize(h.checkAndThrowString(String(description), 'description', 1, 1000))
    : '';
  const cleanNotes = notes
    ? h.sanitize(h.checkAndThrowString(String(notes), 'notes', 1, 500))
    : '';

  const cleanStart  = parseAndValidateDatetime(pickupStartTime, 'pickupStartTime');
  const cleanEnd    = parseAndValidateDatetime(pickupEndTime,   'pickupEndTime');
  const cleanExpiry = parseAndValidateDatetime(expirationTime,  'expirationTime');
  ...
  const newListing = {
    ...
    notes:        cleanNotes,
    pickupAddress: {
      street:  h.sanitize(cleanStreet),
      city:    h.sanitize(cleanCity),
      state:   cleanState,
      zipCode: cleanZip,
    },
    status:          'active',
...
export const updateListing = async (listingId, donorId, updateData) => {
  ...
  if (updateData.foodCategory) {
    updateFields.foodCategory = parseAndValidateCategory(String(updateData.foodCategory));
  }
...
export const deleteListing = async (listingId, donorId) => {
  ...
  const result = await listings.deleteOne({ _id: new ObjectId(cleanListingId) });
