...
router.route("/admin/dashboard").get(requireRole("admin"), async (req, res) => {
  try {
    ...
    const [totalUsers, totalListings, totalTransactions, openComplaints] =
      await Promise.all([
...
router
  ...
  .post(requireRole("admin"), async (req, res) => {
    try {
      ...
      await suspendUser(cleanId, req.session.user._id);
...
router
  ...
  .post(requireRole("admin"), async (req, res) => {
    try {
      const cleanId = xss(req.params.id);
      await unsuspendUser(cleanId, req.session.user._id);
      return res.redirect("/admin/users");
    } catch (e) {
      return res.status(400).render("error", {
        pageTitle: "Error",
...
router
  ...
  .get(requireRole("admin"), async (req, res) => {
    try {
      const cleanId = xss(req.params.id);
      const complaint = await getComplaintById(cleanId);
...
router
  .route("/admin/complaints/:id/resolve")
  .post(requireRole("admin"), async (req, res) => {
    try {
      const cleanId = xss(req.params.id);
      await resolveComplaint(cleanId, req.session.user._id);
